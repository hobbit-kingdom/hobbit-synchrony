...
bool Client::onUserCreate()
{
	...
	std::ifstream file("server_ip.txt"); // Replace "example.txt" with your file name

	if (!file.is_open()) std::cout << "Error: Could not open server_ip.txt." << std::endl;
	else
	{
		if (std::getline(file, ip))
			std::cout << "Sever ip is: " << ip << std::endl;
		else
			std::cout << "Error: Could not read the ip from file." << std::endl;
	}
